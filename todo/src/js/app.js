;(function () {
    ...
    Todo.prototype.bindEvents = function () {
        ...
        this.$parent.addEventListener('click', function (e) {
            ...
            if(e.target.classList.contains('note')) {
                ...
                let id = e.target.parentNode.parentNode.getAttribute('data-id');
                self.updateListItem(id, theTarget);
    ...
    Todo.prototype.updateListItem = function(id, theTarget){
        let todoItems = this.todoItems;


        let $update = document.createElement('input');
        $update.setAttribute('class', 'create-todo');
        theTarget.parentNode.replaceChild($update, theTarget);
        $update.focus();


        $update.addEventListener('blur', function (e) {
            let newText = $update.value;

            for(todoObj of todoItems) {
                if(todoObj.id == id) {
                    todoObj.title = newText;
                    break;
                }
            }
        ...
        $update.addEventListener('keyup', function (e) {
            // Check for enter!
            if (e.which === 13) {
             let newText = $update.value;
